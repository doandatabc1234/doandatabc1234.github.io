    ...
    if ( !is_array( $liked_shops ) ) {
      $liked_shops = array();
		...
		if ( !in_array( $vendor_id, $liked_shops ) ) {
			...
			echo json_encode(array( 'added' => true, 'message' => esc_html__('Favourite Shop', 'handystore'), 'title' => esc_html__('Remove this shop from your Favourites', 'handystore') ));
		} else {
			...
			update_user_option( $user_id, "_liked_vendors", $liked_shops );
			echo json_encode(array( 'added' => false, 'message' => esc_html__('Add to Favourites', 'handystore'), 'title' => esc_html__('Add this shop to your Favourites', 'handystore') ));
		}

	} else {
		ob_start();
			$message = sprintf( __('Only registered users can add vendor shop to Favourite List. Please <a rel="nofollow" href="%s">log in or register here</a>!', 'handystore'), esc_url( get_permalink( get_option('woocommerce_myaccount_page_id') ) ));
...
function pt_output_favourite_button( $vendor_id ) {
  if ( is_user_logged_in() ) { // user is logged in
    ...
    if ( !in_array( $vendor_id, $liked_shops ) ) { // like the post
      $class = '';
      $title = esc_html__('Add this shop to your Favourites', 'handystore');
			$msg = esc_html__('Add to Favourites', 'handystore');
    } else {
      $class = ' added';
      $title = esc_html__('Remove this shop from your Favourites', 'handystore');
			$msg = esc_html__('Favourite Shop', 'handystore');
    }
	} else {
		$class = '';
		$title = esc_html__('Add this shop to your Favourites', 'handystore');
	...
	<div class="favourite-wrapper">
		<a href="#" class="button pt-favourite-vendor<?php echo esc_attr($class);?>" data-vendor_id="<?php echo esc_attr($vendor_id); ?>" title="<?php echo esc_attr($title); ?>">
...
function pt_my_account_favourite_list() {
    ...
    $already_liked_shops = get_user_option( "_liked_vendors", $user_id );
		if ( is_array($already_liked_shops) && count($already_liked_shops)!=0 ) {
			...
			echo '<h3>'.esc_html__('Favourite Vendors:', 'handystore').'</h3>';
			foreach ($already_liked_shops as $vendor_id) {
				...
				if ($products_count && $products_count>0) echo '<span class="products-count"><i class="fa fa-shopping-basket" aria-hidden="true"></i>'.esc_html__( 'Products: ', 'handystore' ).$products_count.'</span>';
				if ($products_onsale_count && $products_onsale_count>0) echo '<span class="products-onsale-count"><i class="fa fa-tag" aria-hidden="true"></i>'.esc_html__( 'On-Sale: ', 'handystore' ).$products_onsale_count.'</span>';
				echo '</div>';
				echo '<a href="#" class="pt-favourite-vendor remove-vendor" data-vendor_id="'.esc_attr($vendor_id).'"><i class="fa fa-times" aria-hidden="true"></i>'.esc_html__('Remove from Favourites', 'handystore').'</a>';
				echo '</div></div></div>';
			}
			unset($vendor_id);
			echo '</div>';
