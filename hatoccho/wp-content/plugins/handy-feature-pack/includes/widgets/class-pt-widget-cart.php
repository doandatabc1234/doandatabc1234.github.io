    ...
    if (handy_get_option('catalog_mode') === 'on') return false;
		...
		<div class="cart-excerpt">
			<div class="excerpt-wrapper">
				<p class="message"><?php echo sprintf(_n('Bạn có 1 sản phẩm trong giỏ hàng','Bạn có %d sản phẩm trong giỏ hàng', $woocommerce->cart->cart_contents_count, 'handy-feature-pack'), $woocommerce->cart->cart_contents_count);?></p>
				<a class="view-cart" href="<?php echo $woocommerce->cart->get_cart_url(); ?>" title="<?php _e('View your shopping cart', 'handy-feature-pack'); ?>">
					<?php _e('chi tiết', 'handy-feature-pack'); ?><i class="fa fa-angle-right"></i>
				</a>
				<p class="total"><?php _e('Tổng phụ: ', 'handy-feature-pack');?><?php echo $woocommerce->cart->get_cart_subtotal();?></p>
				<a href="<?php echo esc_url($woocommerce->cart->get_checkout_url()) ?>" class="button" title="<?php _e( 'kiểm tra', 'handy-feature-pack' ) ?>"><?php _e( 'kiểm tra', 'handy-feature-pack' ) ?></a>
			</div>
		</div>
		<?php
		} else {
